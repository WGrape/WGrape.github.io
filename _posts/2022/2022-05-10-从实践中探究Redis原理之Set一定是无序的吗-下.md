...
## <span id="83">3、实际业务中是否应该使用IntSet结构</span>
可能在有些时候，我们会非常明确当前业务场景下是可以IntSet结构的，但是还是要谨慎使用，或者不使用。因为Redis只是为了优化自身的存储，为Set类型提供了这样的一个特性（IntSet结构），而对于我们这些业务使用方来说，一旦选择使用这样的特性，可能就埋下了定时炸弹。
